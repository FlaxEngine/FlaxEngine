...
#define CONVERT_FLAGS_FLAGS_2_STR(value) if(static_cast<int32>(Flags & GPUTextureFlags::value)) { if (flags.HasChars()) flags += TEXT('|'); flags += TEXT(#value); }
...
bool GPUTexture::Init(const GPUTextureDescription& desc)
{
    ...
    if (desc.IsDepthStencil())
    {
        ...
        if (static_cast<int32>(desc.Flags & GPUTextureFlags::ReadOnlyDepthView) && !device->Limits.HasReadOnlyDepth)
        {
    ...
    {
        if (static_cast<int32>(desc.Flags & GPUTextureFlags::ReadOnlyDepthView))
        {
