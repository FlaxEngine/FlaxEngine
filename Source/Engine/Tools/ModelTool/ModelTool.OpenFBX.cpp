...
struct OpenFbxImporterData
{
    ...
    int32 AddMaterial(ImportedModelData& result, const ofbx::Material* mat)
    {
        ...
        if (index == -1)
        {
            ...
            if (mat && static_cast<int32>(result.Types & ImportDataTypes::Materials))
            {
...
bool ProcessMesh(ImportedModelData& result, OpenFbxImporterData& data, const ofbx::Mesh* aMesh, MeshData& mesh, String& errorMsg, int32 triangleStart, int32 triangleEnd)
{
    ...
    if (skin && skin->getClusterCount() > 0 && static_cast<int32>(result.Types & ImportDataTypes::Skeleton))
    {
    ...
    // Blend Shapes
    if (blendShape && blendShape->getBlendShapeChannelCount() > 0 && static_cast<int32>(result.Types & ImportDataTypes::Skeleton) && data.Options.ImportBlendShapes)
    {
...
bool ModelTool::ImportDataOpenFBX(const char* path, ImportedModelData& data, Options& options, String& errorMsg)
{
    ...
    if (!context)
    {
        ...
        if (static_cast<int32>(data.Types & ImportDataTypes::Geometry))
        ...
        if (static_cast<int32>(data.Types & ImportDataTypes::Textures))
        {
            String outputPath;
    ...
    if (static_cast<int32>(data.Types & ImportDataTypes::Geometry) && context->Scene->getMeshCount() > 0)
    {
    ...
    // Import animations
    if (static_cast<int32>(data.Types & ImportDataTypes::Animations))
    {
