...
bool ProcessMesh(ImportedModelData& result, AssimpImporterData& data, const aiMesh* aMesh, MeshData& mesh, String& errorMsg)
{
    ...
    if (aMesh->mNumBones > 0 && aMesh->mBones && static_cast<int32>(result.Types & ImportDataTypes::Skeleton))
    {
    ...
    // Blend Shapes
    if (aMesh->mNumAnimMeshes > 0 && static_cast<int32>(result.Types & ImportDataTypes::Skeleton) && data.Options.ImportBlendShapes)
    {
...
bool ImportMaterials(ImportedModelData& result, AssimpImporterData& data, String& errorMsg)
{
    ...
    for (uint32 i = 0; i < materialsCount; i++)
    {
        ...
        if (static_cast<int32>(result.Types & ImportDataTypes::Materials))
        {
            aiColor3D aColor;
            ...
            if (static_cast<int32>(result.Types & ImportDataTypes::Textures))
            {
...
bool ModelTool::ImportDataAssimp(const char* path, ImportedModelData& data, Options& options, String& errorMsg)
{
    ...
    if (!context)
    {
        ...
        bool importMeshes = static_cast<int32>(data.Types & ImportDataTypes::Geometry) != 0;
        bool importAnimations = static_cast<int32>(data.Types & ImportDataTypes::Animations) != 0;
    ...
    // Import geometry
    if (static_cast<int32>(data.Types & ImportDataTypes::Geometry) && context->Scene->HasMeshes())
    {
    ...
    // Import skeleton
    if (static_cast<int32>(data.Types & ImportDataTypes::Skeleton))
    {
    ...
    // Import animations
    if (static_cast<int32>(data.Types & ImportDataTypes::Animations) && context->Scene->HasAnimations())
    {
    ...
    // Import nodes
    if (static_cast<int32>(data.Types & ImportDataTypes::Nodes))
    {
