...
uint32 Crc::MemCrc32(const void* data, int32 length, uint32 crc)
{
    // Based on the Slicing-by-8 implementation found here:
    // http://slicing-by-8.sourceforge.net/
    ...
    const uint8* __restrict ptr = (uint8*)data;

    // First we need to align to 32-bits
    int32 initBytes = (int32)(Math::AlignUp<uint64>((uint64)ptr, 4) - (uint64)ptr);
