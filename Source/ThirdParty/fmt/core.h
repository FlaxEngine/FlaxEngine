...
#if !defined(FMT_USE_STRING_VIEW) || FMT_USE_STRING_VIEW
...
template <typename Container>
inline Container &get_container(fmt::back_insert_iterator<Container> it) {
  typedef fmt::back_insert_iterator<Container> bi_iterator;
...
class basic_string_view {
 ...
 public:
  ...
  basic_string_view(const Char *s)
    : data_(s), size_(StringUtils::Length(s)) {}
...
init<C, const void*, named_arg_type>
  ...
  memcpy(val.data, &arg, sizeof(arg));
...
struct buffer_context {
  typedef basic_format_context<
    fmt::back_insert_iterator<internal::basic_buffer<Char>>, Char> type;
};
...
typename std::enable_if<
    is_contiguous<Container>::value, fmt::back_insert_iterator<Container>>::type
  vformat_to(
    fmt::back_insert_iterator<Container> out,
    const S &format_str,
...
inline typename std::enable_if<
  ...
  fmt::back_insert_iterator<Container>>::type
    format_to(fmt::back_insert_iterator<Container> out, const S &format_str,
              const Args &... args) {
