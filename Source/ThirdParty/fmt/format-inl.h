...
const uint32_t basic_data::POWERS_OF_10_32[] = {
  1, FMT_POWERS_OF_10(1)
};

const uint32_t basic_data::ZERO_OR_POWERS_OF_10_32[] = {
  0, FMT_POWERS_OF_10(1)
};

const uint64_t basic_data::ZERO_OR_POWERS_OF_10_64[] = {
  0,
  FMT_POWERS_OF_10(1),
...
const uint64_t basic_data::POW10_SIGNIFICANDS[] = {
...
const int16_t basic_data::POW10_EXPONENTS[] = {
...
const char basic_data::FOREGROUND_COLOR[] = "\x1b[38;2;";
const char basic_data::BACKGROUND_COLOR[] = "\x1b[48;2;";
...
FMT_FUNC fp get_cached_power(int min_exponent, int &pow10_exponent) {
  ...
  int index = static_cast<int>(ceil(
...
struct prettify_handler {
  ...
  ~prettify_handler() {
    FMT_ASSERT(buf.size() >= to_unsigned(size), "invalid buffer size");
    buf.resize(to_unsigned(size));
  }

  template <typename F>
  void insert(ptrdiff_t pos, ptrdiff_t n, F f) {
    memmove(data + pos + n, data + pos, to_unsigned(size - pos));
    f(data + pos);
    size += n;
  }

  void insert(ptrdiff_t pos, char c) {
    memmove(data + pos + 1, data + pos, to_unsigned(size - pos));
    data[pos] = c;
    ++size;
  }
